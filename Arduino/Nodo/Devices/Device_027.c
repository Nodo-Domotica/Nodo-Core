...
#define DEVICE_NAME_027 "Servo"
boolean Device_027(byte function, struct NodoEventStruct *event, char *string)
  {
  boolean success=false;

  #ifdef DEVICE_027_CORE
  switch(function)
    {
    case DEVICE_COMMAND:
      {
    ...
    case DEVICE_MMI_IN:
      ...
      if(GetArgv(string,TempStr,1))
        {
        if(strcasecmp(TempStr,DEVICE_NAME_027)==0)
          {
              ...
              if(event->Par1>0 && event->Par1<5 && event->Par2>=0 && event->Par2<=180)            
                {
                  ...
                  event->Command = 27; // Device nummer
                  success=true;
                }
              }
            }
