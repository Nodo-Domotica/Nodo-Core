//#######################################################################################################
//#################################### Device-028 4-voudige PulseCounter #################################
//#######################################################################################################
...
boolean Device_028(byte function, struct NodoEventStruct *event, char *string)
{
               ...
               if(event->Par1>0 && event->Par1<5 && event->Par2>0 && event->Par2<=USER_VARIABLES_MAX-1)            
                 {
                  ...
                  event->Command = 28; // Device nummer
                  success=true;
                }
              }
            }
  ...
  case DEVICE_MMI_OUT:
    {
      if(event->Command==DEVICE_ID)
      {
        strcpy(string,DEVICE_NAME);
        strcat(string," ");
