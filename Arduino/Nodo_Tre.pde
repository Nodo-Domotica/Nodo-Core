...
#define VERSION        043        // Nodo Version nummer:
                                  // Major.Minor.Patch
...
prog_char PROGMEM Cmd_021[]="SendSignal";
...
prog_char PROGMEM Cmd_037[]="VariableWiredAnalog";
...
#define CMD_VARIABLE_WIRED_ANALOG       37
...
#define ERROR_08                       208
#define ERROR_09                       209
#define LAST_VALUE                     209 // laatste VALUE uit de commando tabel
#define COMMAND_MAX                    210 // aantal commando's (dus geteld vanaf 0)
...
// Analoge poorten:
#define TempSensor                  6   // Analog 6 temperatuursensor
#define RedButton                   7   // Knop factory reset
...
struct Settings
  {
  int     Version;
  byte    WiredInputThreshold[WIRED_PORTS], WiredInputSmittTrigger[WIRED_PORTS], WiredInputPullUp[WIRED_PORTS],WiredInputRange[WIRED_PORTS];
...
Server EventServer(1024);                           // Server class voor Events
Client EventClient=false;                           // Client class voor Events
...
void setup() 
  ...
  Ethernet.begin(Ethernet_MAC_Address, S.Nodo_IP);    // MAC en IP adres van de Nodo
  Server EventServer(S.Event_Port);                   // Server class voor ontvangst van Events
...
void loop() 
  ...
  while(true)
    ...
    if(S.Terminal_Enabled==VALUE_ON)
      {        
      if(!TerminalConnected && TerminalServer.available())
        {
