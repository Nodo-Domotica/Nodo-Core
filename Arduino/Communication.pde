  ...
  if(EventClient) 
    ...
    while((InByteIPCount<INPUT_BUFFER_SIZE) && TimeoutTimer>millis() )
      ...
      if(InByteIP==0x0a)
        ...
        // regel is compleet. 
        // Serial.print("*** Received IP: ");Serial.println(Buffer);//??? debug
        ...
        if(EGState==2)
          ...
          if(strcasecmp(InputBuffer,PROGMEM2str(Text_20))==0) // "quintessence" 
            ...
            // ga naar volgende state: Haal cookie en verwerk deze
            EGState=1;
            }
          }
        Buffer[0]=0;          
...
boolean EventGhostSend(char* event, byte* IP)
  ...
  Client EGclient(&IP[0], S.Event_Port);
