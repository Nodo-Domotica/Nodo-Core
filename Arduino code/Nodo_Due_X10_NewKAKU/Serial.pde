...
unsigned long Receive_Serial(void)
  ...
  if(!y || y>=RANGE_EVENT)return Event;
  
  E.Port=CMD_PORT_SERIAL;
  E.Type=CMD_TYPE_COMMAND;
  E.Content=Event; 
  Cmd=(Event>>16)&0xff;
  Par1=(Event>>8)&0xff;
  Par2=Event&0xff;
     ...
     case CMD_RAWSIGNAL_PUT:
      ...
      case CMD_STATUS:
         ...
         if(GetStatus(&Cmd,&Par1,&Par2)) // let op: call by reference !
           ...
           PrintCode(Event);// ...maar alleen weergegeven.
           PrintTerm();
           }
         else
           error=true;
