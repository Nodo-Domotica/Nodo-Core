...
public class NodoDueManager extends SingleFrameApplication {
    ...
    public static void showDialog(String key, Object... obj) {
        final NodoDueManagerView view = getApplication().getView();
        if ( view == null ) {
            System.out.println("[ERROR] " + getMessage(key, obj));
        } else {
            JOptionPane.showMessageDialog(view.getComponent(), getMessage(key, obj), getMessage("warningTitle"), JOptionPane.WARNING_MESSAGE);
        }
        if (obj != null) {
            for (int i = 0; i < obj.length; i++) {
                if (obj[i] != null && obj[i] instanceof Exception) {
